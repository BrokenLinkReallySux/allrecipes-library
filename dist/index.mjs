...
async function getRecipe(url) {
    const response = await axios.get(url), text = response.data, { window } = new JSDOM(text), { document } = window;
    // @ts-ignore
...
async function searchOnce(query, last) {
    ...
    const response = await axios.get(searchURL), text = response.data, { window } = new JSDOM(text), { document } = window;
    // see get-recipe.ts
    // @ts-ignore
    const $ = ("fn" in jquery ? jquery : jquery(window));
    let recipes = [];
...
export { getRecipe, search, searchOnce };
